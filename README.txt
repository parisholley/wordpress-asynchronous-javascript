...
Tags: async,headjs,asynchronous,performance,javascript,css
Requires at least: 3.5
Tested up to: 3.5
Stable tag: trunk

Improve page load performance by asynchronously loading javascript and CSS files using head.js
...
In your theme, use the static AsynchronousAssets::wp_enqueue_async_script() method, which is API compatible with wp_enqueue_script($handle, $src, $deps). Using this method will cause the $src to load ONLY when scripts with the handles defined in $deps have been loaded.
...
= 0.0.1 =
* Initial release, no CSS support yet. Me so sleepy :(